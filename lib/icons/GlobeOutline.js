...
export const GlobeOutline = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-GlobeOutline-${count}`}"
      aria-describedby="${`title-GlobeOutline-${count}`}"
    >
      <title id="${`title-GlobeOutline-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-GlobeOutline-${count}`}">${desc}</desc>
          `
        : html``}
      <path
