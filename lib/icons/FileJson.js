...
export const FileJson = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-FileJson-${count}`}"
      aria-describedby="${`title-FileJson-${count}`}"
    >
      <title id="${`title-FileJson-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-FileJson-${count}`}">${desc}</desc>
          `
        : html``}
