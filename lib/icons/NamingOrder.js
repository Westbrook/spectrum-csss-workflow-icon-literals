...
export const NamingOrder = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-NamingOrder-${count}`}"
      aria-describedby="${`title-NamingOrder-${count}`}"
    >
      <title id="${`title-NamingOrder-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-NamingOrder-${count}`}">${desc}</desc>
          `
        : html``}
      <path
