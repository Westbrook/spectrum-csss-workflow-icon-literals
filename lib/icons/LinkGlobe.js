...
export const LinkGlobe = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-LinkGlobe-${count}`}"
      aria-describedby="${`title-LinkGlobe-${count}`}"
    >
      <title id="${`title-LinkGlobe-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-LinkGlobe-${count}`}">${desc}</desc>
          `
        : html``}
      <path
