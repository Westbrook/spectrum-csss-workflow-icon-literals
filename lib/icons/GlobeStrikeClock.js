...
export const GlobeStrikeClock = ({
  width = 24,
  height = 24,
  hidden = false,
  title = 'Globe Strike Clock',
  desc,
} = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-GlobeStrikeClock-${count}`}"
      aria-describedby="${`title-GlobeStrikeClock-${count}`}"
    >
      <title id="${`title-GlobeStrikeClock-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-GlobeStrikeClock-${count}`}">${desc}</desc>
          `
        : html``}
      <path
