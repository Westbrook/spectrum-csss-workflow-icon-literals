...
export const SpotHeal = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-SpotHeal-${count}`}"
      aria-describedby="${`title-SpotHeal-${count}`}"
    >
      <title id="${`title-SpotHeal-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-SpotHeal-${count}`}">${desc}</desc>
          `
        : html``}
      <path
