...
export const JourneyAction = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-JourneyAction-${count}`}"
      aria-describedby="${`title-JourneyAction-${count}`}"
    >
      <title id="${`title-JourneyAction-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-JourneyAction-${count}`}">${desc}</desc>
          `
        : html``}
      <path
