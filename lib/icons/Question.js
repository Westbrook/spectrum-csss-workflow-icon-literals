...
export const Question = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-Question-${count}`}"
      aria-describedby="${`title-Question-${count}`}"
    >
      <title id="${`title-Question-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-Question-${count}`}">${desc}</desc>
          `
        : html``}
      <path
