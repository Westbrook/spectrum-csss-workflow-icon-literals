...
export const GearsAdd = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-GearsAdd-${count}`}"
      aria-describedby="${`title-GearsAdd-${count}`}"
    >
      <title id="${`title-GearsAdd-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-GearsAdd-${count}`}">${desc}</desc>
          `
        : html``}
      <path
