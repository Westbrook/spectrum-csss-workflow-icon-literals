...
export const TextLetteredUpperCase = ({
  width = 24,
  height = 24,
  hidden = false,
  title = 'Text Lettered Upper Case',
  desc,
} = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-TextLetteredUpperCase-${count}`}"
      aria-describedby="${`title-TextLetteredUpperCase-${count}`}"
    >
      <title id="${`title-TextLetteredUpperCase-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-TextLetteredUpperCase-${count}`}">${desc}</desc>
          `
        : html``}
