...
export const EmailExclude = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-EmailExclude-${count}`}"
      aria-describedby="${`title-EmailExclude-${count}`}"
    >
      <title id="${`title-EmailExclude-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-EmailExclude-${count}`}">${desc}</desc>
          `
        : html``}
      <path
