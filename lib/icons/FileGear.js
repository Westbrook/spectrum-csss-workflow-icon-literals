...
export const FileGear = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-FileGear-${count}`}"
      aria-describedby="${`title-FileGear-${count}`}"
    >
      <title id="${`title-FileGear-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-FileGear-${count}`}">${desc}</desc>
          `
        : html``}
      <path
      ...
      />
      <path
        d="M20 4L8 16h12zm18 0H24v14a2 2 0 01-2 2H8v22a2 2 0 002 2h14.52A13.99 13.99 0 0140 22.587V6a2 2 0 00-2-2z"
      />
    </svg>
  `;
};
