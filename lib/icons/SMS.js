...
export const Sms = ({ width = 24, height = 24, hidden = false, title = 'Sms', desc } = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-SMS-${count}`}"
      aria-describedby="${`title-SMS-${count}`}"
    >
      <title id="${`title-SMS-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-SMS-${count}`}">${desc}</desc>
          `
        : html``}
      <path
