...
export const UploadToCloudOutline = ({
  width = 24,
  height = 24,
  hidden = false,
  title = 'Upload To Cloud Outline',
  desc,
} = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-UploadToCloudOutline-${count}`}"
      aria-describedby="${`title-UploadToCloudOutline-${count}`}"
    >
      <title id="${`title-UploadToCloudOutline-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-UploadToCloudOutline-${count}`}">${desc}</desc>
          `
        : html``}
      <path
        d="M24.313 17.11a.5.5 0 00-.626 0l-5.451 5.524a.785.785 0 00-.236.56.8.8 0 00.8.806H22v19a1 1 0 001 1h2a1 1 0 001-1V24h3.2a.8.8 0 00.8-.806.785.785 0 00-.236-.56z"
      />
      <path
