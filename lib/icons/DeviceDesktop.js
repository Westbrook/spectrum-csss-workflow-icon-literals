...
export const DeviceDesktop = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-DeviceDesktop-${count}`}"
      aria-describedby="${`title-DeviceDesktop-${count}`}"
    >
      <title id="${`title-DeviceDesktop-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-DeviceDesktop-${count}`}">${desc}</desc>
          `
        : html``}
      <path
