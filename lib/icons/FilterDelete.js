...
export const FilterDelete = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-FilterDelete-${count}`}"
      aria-describedby="${`title-FilterDelete-${count}`}"
    >
      <title id="${`title-FilterDelete-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-FilterDelete-${count}`}">${desc}</desc>
          `
        : html``}
      <path
