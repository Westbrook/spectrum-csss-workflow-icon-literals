...
export const Abc = ({ width = 24, height = 24, hidden = false, title = 'Abc', desc } = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-ABC-${count}`}"
      aria-describedby="${`title-ABC-${count}`}"
    >
      <title id="${`title-ABC-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-ABC-${count}`}">${desc}</desc>
          `
        : html``}
      <path
