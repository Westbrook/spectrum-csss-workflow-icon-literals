...
export const AddToSelection = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-AddToSelection-${count}`}"
      aria-describedby="${`title-AddToSelection-${count}`}"
    >
      <title id="${`title-AddToSelection-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-AddToSelection-${count}`}">${desc}</desc>
          `
        : html``}
      <path
