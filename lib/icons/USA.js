...
export const Usa = ({ width = 24, height = 24, hidden = false, title = 'Usa', desc } = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-USA-${count}`}"
      aria-describedby="${`title-USA-${count}`}"
    >
      <title id="${`title-USA-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-USA-${count}`}">${desc}</desc>
          `
        : html``}
      <path
