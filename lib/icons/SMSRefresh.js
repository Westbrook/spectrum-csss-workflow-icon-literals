...
export const SMSRefresh = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-SMSRefresh-${count}`}"
      aria-describedby="${`title-SMSRefresh-${count}`}"
    >
      <title id="${`title-SMSRefresh-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-SMSRefresh-${count}`}">${desc}</desc>
          `
        : html``}
      <path
