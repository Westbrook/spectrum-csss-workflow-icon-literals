...
export const Hand1 = ({ width = 24, height = 24, hidden = false, title = 'Hand1', desc } = {}) => {
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-Hand1-${count}`}"
      aria-describedby="${`title-Hand1-${count}`}"
    >
      <title id="${`title-Hand1-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-Hand1-${count}`}">${desc}</desc>
          `
        : html``}
      <path
