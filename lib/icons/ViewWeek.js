...
export const ViewWeek = ({
  ...
  return html`
    <svg
      ...
      role="img"
      fill="currentColor"
      aria-labelledby="${`title-ViewWeek-${count}`}"
      aria-describedby="${`title-ViewWeek-${count}`}"
    >
      <title id="${`title-ViewWeek-${count}`}">${title}</title>
      ${desc
        ? html`
            <desc id="${`desc-ViewWeek-${count}`}">${desc}</desc>
          `
        : html``}
